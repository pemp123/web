...
(function() {
  ...
  var connectionString;
    var gameSocket;
     connectionString = 'wss://' + window.location.host + '/game_2048';
...
var data;
    ...
    { 
      data = {
                "event": "move",
                "message": JSON.stringify({"grid" : grid})
            }
	...
	var Metrics = (function() {
		var PIXEL_RATIO = (function () {
			//noinspection JSUnresolvedVariable
			 var ctx = document.getElementById("g2048").getContext("2d"),
		...
		var body = document.getElementById("player1");
    //var body2 = document.getElementById("player2");
        ...
        var canvas;
        if(id == 2)
          canvas = document.getElementById("g2048");
        if(id == 1)
				 canvas = document.getElementById("score_g2048");

        // if(id == 4)
          
        //   canvas = document.getElementById("g20482");
        // if(id == 3)
        // {
        //   console.log("111");
				//  canvas = document.getElementById("score_g20482");
        // }
...
//				context.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
			...
			getWidth: function() {
				var e = document.documentElement,
					width = window.innerWidth/1.5|| e.clientWidth || body.clientWidth6;

				return width;
			},
			getHeight: function() {
				var e = document.documentElement,
					height = window.innerHeight/1.5|| e.clientHeight|| body.clientHeight;
		...
		return {
			createCanvas: function(width, height,id) {
				return Metrics.createCanvas(width, height,id);
			},
			getPixelRatio: function() {
  ...
  var Metrics2 = (function() {
		var PIXEL_RATIO = (function () {
			//noinspection JSUnresolvedVariable
			 var ctx = document.getElementById("g20482").getContext("2d"),
				dpr = window.devicePixelRatio || 1,
				bsr = ctx.webkitBackingStorePixelRatio ||
					ctx.mozBackingStorePixelRatio ||
					ctx.msBackingStorePixelRatio ||
...
//				context.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
			...
			getWidth: function() {
				var e = document.documentElement,
					width = window.innerWidth/1.5 || e.clientWidth || body2.clientWidth;

				return width;
			},
			getHeight: function() {
				var e = document.documentElement,
					height = window.innerHeight/1.5|| e.clientHeight|| body2.clientHeight;
  ...
  var Renderer2 = (function() {
		...
		var Renderer = {
			render: function(canvas, context, grid) {
				...
				var cellHeight = Metrics2.getCellHeight(canvas, grid);
				var cellWidth = Metrics2.getCellWidth(canvas, grid);
        ...
        //this.drawCells(context, grid, cellWidth, cellHeight);
			...
			showGameOver: function(canvas, context) {
				...
				// Draw "Game Over!" text
				context.font = "bold " + Metrics2.getHeaderSize() + "px Helvetica Neue";
				context.textAlign = "center";
				context.textBaseline = "middle";
				context.fillStyle = assets.gameOver.textColor;
				context.fillText("Game Over!", (canvas.width / 2), (canvas.height / 2), canvas.width);
			...
			showRestart: function(canvas, context) {
				var fontSize = Metrics2.getResetSize();
				var buttonWidth = Math.floor(fontSize * 6.67);
				var buttonHeight = Math.floor(buttonWidth / 2.5);
				var margin = Math.floor(buttonWidth / 10);
				var buttonX = (canvas.width / 2) - (buttonWidth / 2);
	...
	window.Application = (function() {
		...
		var gameCanvas, gameContext, scoreCanvas, scoreContext,scoreCanvas2, scoreContext2,gameCanvas2, gameContext2;
      ...
      var scoreCanvasHeight2 = 100 / Metrics2.getPixelRatio();
			var size2 = Metrics2.getSize();
			size2 = ((Metrics2.getHeight() - scoreCanvasHeight2) <= Metrics2.getWidth()) ? (Metrics2.getHeight() - scoreCanvasHeight2) : size;

			// Get the score canvas ready
			scoreCanvas = Metrics.createCanvas(size, scoreCanvasHeight,1);
			scoreContext = scoreCanvas.getContext("2d");

			// Get the main canvas ready
			gameCanvas = Metrics.createCanvas(size, size,2);
			gameContext = gameCanvas.getContext("2d");

      			// Get the score canvas ready
			 scoreCanvas2 = Metrics2.createCanvas(size, scoreCanvasHeight2,3);
			 scoreContext2 = scoreCanvas2.getContext("2d");

			// // Get the main canvas ready
			 gameCanvas2 = Metrics2.createCanvas(size2, size2,4);
			 gameContext2 = gameCanvas2.getContext("2d");

			// Append the canvases to the DOM
			 var body2 = document.getElementById("player2");
			 body2.appendChild(scoreCanvas2);
			 body2.appendChild(gameCanvas2);
    ...
    game2 = {
      ...
      Renderer.render(gameCanvas2, gameContext2, game2.grid);
			// Draw the scores
			Renderer.renderScores(scoreCanvas, scoreContext, game.data.score, game.data.topScore);
      Renderer.renderScores(scoreCanvas2, scoreContext2, game2.data.score, game2.data.topScore);
